...
**Gantt Timeline: date axis + dependency arrows (2026-08-03).** Added a real predecessor-link (`predecessors: [codes]`) field to every activity across all three parsers (`analyzeXER`, `analyzeCSVTasks`, `analyzeMspXml`) — previously only a `hasPredecessor`/`hasSuccessor` boolean existed, not enough to draw an actual connector. `renderGantt()` now renders a month-tick date axis (day-tick fallback when the file has no calendar dates) and an SVG overlay of elbow connector lines between each activity's real predecessor(s) and itself, using the exact same leftPct/widthPct geometry already computed for the bars so everything lines up without a DOM-measurement pass. Verified against the real sample schedule via curl (`/api/analyze` response's `activities[].predecessors` matches the file's actual 5-link chain exactly) and Playwright (6 bars, 5 arrow paths, 8 axis ticks all present in the DOM, zero real console errors). Caught and fixed one real rendering bug during QC: the arrows SVG deliberately uses a non-uniform viewBox (x in %, y in row-count, `preserveAspectRatio="none"`) so it can be positioned without measuring the DOM — but that distorts anything sized in the same user-space units, which turned a small SVG `<marker>` arrowhead into a huge stretched triangle covering multiple rows. Fixed by dropping the marker (plain connector lines only) and adding `vector-effect: non-scaling-stroke` so line thickness isn't distorted the same way. Scoped to axis + arrows only, per the owner's explicit priority pick — milestone-diamond markers, WBS grouping, and baseline comparison were flagged as separate, larger follow-ups, not built here.

**Gantt Timeline: milestone diamonds, baseline comparison, WBS grouping (2026-08-03).** Built the three follow-ups flagged above. Milestones (`task_type: TT_Mile`) now render as a rotated-square diamond marker instead of a zero-width bar, with a hover tooltip. Non-milestone bars get a second, thinner strip beneath them showing the baseline (`plannedStart`/`plannedEnd`, already computed for Earned Schedule) alongside the actual bar, with a hover tooltip stating the exact drift in days (early/late/on-baseline) — zero new backend fields needed, since baseline dates already existed in the activity model. WBS grouping parses `PROJWBS` in `analyzeXER` (new `wbsGroupFor()` helper mapping `wbs_id` → the nearest-parent phase name, skipping the root project node) and adds a `wbsGroup` field to each activity; `renderGantt()` now renders a collapsible `.gantt-wbs-header` row per group (click to toggle, chevron flips, full re-render — consistent with this codebase's no-incremental-DOM-patching pattern everywhere else) with a `Set`-backed `collapsedWbsGroups` state var. The predecessor-arrow overlay was refactored from positional array indices to a `codeToIndex` map keyed off the currently-visible row slots, so arrows correctly recompute around collapsed groups: verified via Playwright that a full 6-activity/5-dependency chain draws all 5 arrows expanded, and correctly drops to 3 when a 2-activity group collapses (the 2 arrows with an endpoint inside the hidden group are cleanly omitted, not drawn as garbage) — confirmed both by counting DOM `.gantt-arrow-path` elements and inspecting their `d` attributes before and after the toggle. Built a hand-crafted XER test fixture with a real `PROJWBS` table (3 phases) and `wbs_id`-tagged tasks — the repo's existing sample data has no WBS section — and caught one bug in that fixture itself (a task row short one tab-delimited field, so `wbs_id` parsed as `undefined` for one activity) via `awk -F'\t' '{print NF}'` field-count verification before confirming the parsing logic was correct. WBS parsing is XER-only for now; the CSV and MSP-XML parsers don't carry `wbs_id`/WBS-tree data in this codebase's existing field mapping, so they don't populate `wbsGroup` (activities render ungrouped, same as today, until that's added).

**Lead magnet + free public checker (2026-08-02).** `POST /api/public/lead-magnet` verified via curl: each of the four required-field validations (missing role, missing consent) returns the right `400` before any row is written, a valid submission returns `200` and lands a row in `leads` with the exact values submitted, and the gate HTML block (`/blog/dcma-14-point-check-guide`) was confirmed present on that post and absent from `/blog/contractor-baseline-red-flags-checklist` (grepped both rendered pages for the literal "Get the free PDF checklist" button text, not just the shared CSS class name). Knock delivery itself is unverified — no `KNOCK_LEAD_MAGNET_WORKFLOW_KEY` set locally, degrades as designed (lead still saved, `pdf_sent_at` stays null). `POST /api/public/check-schedule` verified via curl (real `.xer` sample → real score/issues, no snapshot/project row created) and its 10/hour rate limit (11 rapid requests → first 9 succeed, then `429`s). `/check` (`free-schedule-check.html`) driven end-to-end with Playwright: file upload → real analysis renders (donut score, top-3 issues color-coded by severity, "+N more" note, sign-up CTA) — caught and fixed two real layout bugs in the process (the drag-and-drop `<label>` collapsing because it was inline by default, and the score digit becoming invisible where it overlapped a same-colored ring segment, fixed by giving the donut a solid inner "hole" behind the number).

**`/check` redesign — conversion-focused rebuild (2026-08-04).** Rebuilt `free-schedule-check.html` from a plain upload form into a full landing page per a detailed design brief: cursor-tracking spotlight background, an always-visible report panel (populated with clearly-labeled sample data before any upload, swapping to real data after), a 4-stage upload state machine (idle/ready/running/done) with real `XMLHttpRequest.upload.onprogress`-driven progress, entrance/stagger animations, and a `prefers-reduced-motion` guard. The brief specified React + TypeScript + Vite + a Phosphor Icons npm package; built it in vanilla HTML/CSS/JS instead and said so before starting — this app is explicitly zero-dependency/no-build-step by design (see top of this doc), and introducing a build toolchain for one page would be a real, hard-to-reverse architecture change. Icons are hand-authored inline SVGs. Wired to the existing, already-live `/api/public/check-schedule` endpoint (mapped its real field names) rather than adding a second route with a different contract. The brief's "every check" pill list was the literal DCMA 14-point list; swapped for the 7 checks the engine actually runs (missing logic, negative float, high float, high duration, hard constraints, invalid dates, out-of-sequence/XER-only), matching this project's standing rule against overclaiming DCMA coverage (see the `dcma-14point-copy-corrections-PROPOSAL.md` history). Verified with Playwright: initial paint shows the disabled CTA with an honest label ("Add a file to check") and the sample report already populated (score 68, 1,240 activities, 17 issues); a real `.xer` upload runs idle→ready→running→done, renders the real score/activities/issues/critical count and real top-3 issue names from the live API, hides the CTA during the running stage, and "Check another schedule" correctly resets to idle with the sample report restored; the sample-schedule button (reusing the same demo dataset as the main app's own "Load sample data") produces a complete real run with no file chosen; keyboard focus reaches the dropzone's file input and the sample button; the mobile breakpoint collapses to a clean single column with no overflow; `prefers-reduced-motion: reduce` renders everything immediately visible with no animation delay. Only console output across every test was the known sandbox-only external-font-fetch limitation (not a real bug, see the LCP entry above). Not yet deployed to production — this is on `claude/new-session-8x1hcw` only; needs the same tested-branch-then-merge-to-`main` step the original `/check` route needed before it goes live.

**Homepage LCP fix — non-blocking Google Fonts (2026-08-04).** A Semrush Core Web Vitals audit flagged the homepage LCP as Poor (4.46s; TBT and CLS were already Good) and named the cause as "Eliminate render-blocking resources" — the Google Fonts `<link rel="stylesheet">` in `<head>` was a synchronous request blocking first paint. Switched it to the standard preload-then-swap pattern (`<link rel="preload" as="style" ... onload="this.onload=null;this.rel='stylesheet'">` with a `<noscript>` fallback for non-JS clients), so the fonts load without blocking the initial render; `font-display: swap` was already set, so text still shows immediately in a fallback font either way. Preconnects to both `fonts.googleapis.com` and `fonts.gstatic.com` were already present, ahead of the stylesheet link — Semrush's separate "Preconnect to Required Origins" flag on the same report looks stale against the live page. Verified the change doesn't break rendering: page loads and the marketing hero/feature grid render correctly with Playwright, no new console errors beyond a font-fetch failure that's a sandbox proxy limitation (this dev container's headless browser can't reach external hosts the same way its shell `curl` can — confirmed `curl -I https://fonts.googleapis.com` returns `200` from the same container) and not caused by this change. Could not get a live before/after LCP number from this environment; recommend re-running the Semrush audit against production after deploy to confirm the drop. "Remove Unused JavaScript" (the ~3,000-line inline `<script>` carrying the whole app for an anonymous marketing-page visitor) is a real, separate finding — a much bigger refactor (splitting critical vs. deferred JS) that wasn't attempted here.
...
**Invalid Dates check, restated-actuals/regressed-progress detection, and the early-slip-no-recovery alert.** Added after reviewing SmartPM's 2026 "State of Construction Scheduling" industry report — three real data-integrity gaps their own research called out (44% of updates carry risky changed-actual-dates, 32% walk back reported progress, and a sustained mid-project SPI dip with no recovery response was their causal study's single strongest predictor of a bad outcome). All three verified with hand-built synthetic XER fixtures, both via direct unit calls to `analyze.js` and via a full curl/Playwright pass against a running server: (1) Invalid Dates — a task with reversed actual dates, one with an actual finish but no actual start, one with a reversed planned baseline, and one with an actual date set in the future all correctly produced one issue each, tagged `invalid dates` and `sev: risk`; also caught and fixed a real edge case where a reversed-date task's duration clamps to zero and gets silently reclassified as a milestone upstream — the check now runs before that branch for exactly this reason, confirmed by re-testing the same fixture. (2) `GET /api/snapshots/compare` — uploaded three snapshots of the same project with a deliberately regressed `percentComplete` on one activity and a deliberately changed `act_start_date` on another between snapshots 1 and 2; the compare response correctly listed both under new `changedActuals`/`regressedProgress` arrays, and correctly showed empty arrays for the (unchanged) snapshot 2→3 pair, confirming it's not just always firing. Screenshotted the two new "Data integrity" cards in the Trends view rendering the real from/to values. (3) The recovery alert — calibrated a synthetic schedule with planned dates in the past relative to "now" so schedule-only SPI computed to ~0.47 (well under the 0.9 threshold); confirmed `available: false` after one snapshot (not enough history), `triggered: true` after 2–3 sustained-low snapshots, and separately unit-tested the pure `computeRecoveryAlert` function's branch logic in isolation (healthy/above-threshold, sustained-low-and-not-recovering, was-low-but-recovering, and one-off-dip-not-sustained all returned the correct `triggered` value and reason text). Confirmed the alert is computed consistently from both `POST /api/analyze` (right after a new upload) and `GET /api/projects/:name/latest` (reopening the project later) via the same shared `computeRecoveryAlertForProject` helper. Screenshotted the resulting banner rendering correctly at the top of the Earned Value panel for a Pro-tier account (both Earned Value and snapshot comparison are Pro-gated features, so the test user's `plan_tier` was flipped directly in the database — Stripe isn't reachable from this sandbox).

**Fix guidance (Pro feature).** Rule-based, not AI-generated — a written remediation guide per DCMA-style check type (`fix-guidance.js`), matched by the same substring convention `analyze.js`'s `subScoresFrom()` already uses. Verified server-side enforcement specifically, not just a frontend hint: `GET /api/issues/:id/fix-guidance` correctly returns `402` for a free-tier account (even though the client-side gate would normally stop the call before it's made — tested by hitting the route directly), `200` with the right category-matched guidance once the same account's `plan_tier` was flipped to Pro directly in the database, `404` for a nonexistent issue id, and `403` when a second Pro-tier account tries to fetch the first account's issue — ownership is checked independently of tier, so being Pro never bypasses the per-user authorization check. Tested three issue categories (missing logic, negative float, hard constraint) and confirmed each returned its own distinct, correctly-matched guidance text rather than a generic fallback. The frontend was screenshotted in both states: a free account clicking "How to fix" sees an inline upgrade card exactly where the issue was clicked; a Pro account sees the real guidance rendered in an accent-bordered card, with two different issues' guidance expanded simultaneously without interfering with each other.
