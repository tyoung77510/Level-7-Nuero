...
**LinkedIn caption formula — SUPERSEDED 2026-07-26, refined 2026-08-04, see changelog below for the live version.** ~~hook line → the problem/story → what Ordo7 does → `Read the full post → https://www.ordo7.pro/blog/<slug>` → `Follow Ordo7 on LinkedIn → https://www.linkedin.com/company/ordo7/` → `Try It For Free → https://ordo7.pro` → 4–5 hashtags.~~ **`#Ordo7` is mandatory in every post, every time** — rotate the remaining 3–4 from: `#BuildInPublic #ProjectControls #Scheduling #SaaS #FounderJourney #DCMA #Bootstrapped #ConstructionTech #PMP`, picked for relevance to that day's topic. **Every Founder Log post has a matching blog entry (`backend/src/blog-content.js`) — the caption must link to it**, now via the first comment (see changelog), not the caption body. Both the blog-link requirement and the hashtag rule were missed on the first live post (Day 003, caught and corrected after publishing — see `founder-log-calendar.md`'s note); don't repeat either gap.

**Changelog (2026-08-04):** LinkedIn's own creator guidance for August 2026 ("Create Content That Starts Conversations," LinkedIn Guide to Creating) names five concrete shifts for content that generates discussion instead of just impressions. Folding the ones that are real refinements — not just restating what's already here — into the formula:

1. **The hook takes one of two real forms, not a generic "problem I solved" opener:** either (a) share an actual perspective or decision Taj made, then ask how others have approached the same situation, or (b) name an actual observation or trend from his real project-controls experience and invite people to add what they're seeing. Both must be real per this doc's own no-fabrication rule — this isn't license to invent a trend to make a better hook.
2. **The closing question is upgraded, not just present.** "A direct question inviting a reply" (2026-07-26) was necessary but not specific enough — a yes/no question still caps engagement at a one-word reply. The question should ask for a specific experience or story ("what's the worst version of this you've seen" beats "does this sound familiar?").
3. **Reply to thoughtful comments — flagged as a manual step, not automated.** LinkedIn's guidance is explicit that replying keeps a thread alive longer than the original post does. The current Zapier LinkedIn connection only exposes `share` and `create_company_update` (confirmed 2026-08-04, no comment-read or comment-reply action available) — the Routine cannot do this itself. This is Taj's own follow-up after a post goes live, not something to build a workaround for.
4. **Follow-up posts revisiting real engagement are a standing option, not a new scheduled slot.** When a post actually gets a substantive comment thread, a later post building on what was said there is worth doing — but only when that engagement is real (same fabrication rule as everything else in this doc: no inventing a "great discussion" to revisit). Use judgment on the day; this isn't added to the 7-slot weekly wheel in `founder-log-calendar.md`.

Days already posted (001–005, 007, 009, 010, and 008 — posted out of order once real content existed) are not retroactively edited, per the standing rule for this doc. Day 011 hasn't posted yet — see the note in `batch-04-linkedin-captions.md` on whether its closing question needs the same upgrade before it fires, following the same "revise if unposted" precedent set in the 2026-07-26 changelog. Effective Day 012 onward for new drafts either way.

**Changelog (2026-07-26):** Real analytics on Day 009 (first post with usable data: 512 impressions, 354 members reached, right audience — Construction 21%, Project Manager/Planning Engineer/Project Controls titles, Senior 39% — but **zero reactions, zero comments, zero reposts**) surfaced two structural problems in the formula above, not a targeting problem: (1) **two-to-three outbound links in the caption body** (blog + LinkedIn company page + ordo7.pro), which LinkedIn's algorithm is known to suppress distribution for, plausibly capping Day 009 at 512 impressions before engagement could even build; (2) **no ask** — the caption pivots from observation straight into product pitch with no question inviting a reply, which lines up with the zero comments. **New formula, effective Day 011 onward:** hook line → the problem/story → what Ordo7 does, one line, no link → a direct question inviting a reply → 3–4 hashtags (`#Ordo7` mandatory). **No URLs typed in the caption body at all** — including the blog link. Per `founder-log-posting-sop.md`, the post is already built as a link-card off `content__submitted_url` (the blog URL), which is a separate field from the caption/commentary text and renders its own rich preview automatically; typing `Read the full post → <URL>` in the caption on top of that was pure redundant risk, not something the reader needed. The one link that has no card backing it — the product ask — moves to a **single first comment**, posted immediately after publishing: `Try Ordo7 free → https://ordo7.pro`. The `Follow Ordo7 on LinkedIn` link is dropped entirely (one ask, not three) — people can follow from the profile directly. Days 001–010 are already live and are not retroactively edited, per the standing rule for this doc; Days 011–012 (`batch-04-linkedin-captions.md`) are rewritten to the new formula since neither had posted yet when this was found.
...
- **DCMA-style issue detection** — 6 of the official DCMA 14-point checks automated today (logic, hard constraints, high float, negative float, high duration, invalid dates), plus out-of-sequence work detection (not a DCMA check, but flagged the same way). Flagged automatically on upload. Included on every plan, including Free. Do not describe this as full 14-point coverage — it isn't yet.
